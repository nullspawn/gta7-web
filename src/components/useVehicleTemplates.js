...
useGLTF.preload(CAR_URLS[0]); useGLTF.preload(CAR_URLS[1]); useGLTF.preload(MOTO_URL)
...
function normalize(scene, targetLen) {
  ...
  root.position.x -= c.x; root.position.z -= c.z; root.position.y -= bb.min.y
  wrap.traverse((o) => {
    if (o.isMesh) {
      o.castShadow = true
