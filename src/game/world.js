import { WORLD, BLOCK, ROAD, rand, randi } from './constants.js'
  ...
  for (let cx = 0; cx < cols; cx++) {
    for (let cy = 0; cy < rows; cy++) {
      ...
      if (Math.random() < 0.12) {
        ...
        for (let t = 0; t < 5; t++)
          trees.push({ x: bx + rand(30, iw - 30), z: bz + rand(30, ih - 30), scale: rand(0.85, 1.3) })
        continue
      }
      const sub = randi(1, 3)
      const pad = 18
...
export function findRoadPoint() {
  for (let i = 0; i < 200; i++) {
    ...
    let x, z
    if (horiz) { x = cx * BLOCK + rand(ROAD, BLOCK); z = cy * BLOCK + ROAD * 0.5 }
    else { x = cx * BLOCK + ROAD * 0.5; z = cy * BLOCK + rand(ROAD, BLOCK) }
